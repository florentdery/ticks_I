...
ticks_standardized <- ticks %>% 
  ...
  dplyr::select(siteID, time ,amblyomma_americanum ) %>%
...
ticks_standardized_w_temps <- ticks_standardized_w_temps %>%
...
ticks_standardized_trimmed <- ticks_standardized_w_temps %>%
  mutate(dplyr::across(BLAN:UKFS,
...
model{
  ...
  for(site in 1:n_site){
    ...
    alpha_temp[site] ~ dnorm(0,tau_add[site])  ## random site effect on K
